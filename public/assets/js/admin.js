...
(function($) {
	...
	if ( $.isFunction( $.fn[ 'dataTable' ] ) ) {
		...
		$.extend(true, $.fn.dataTable.defaults, {
			language: { url: '/vendor/datatables/media/Portuguese-Brasil.json' },
...
(function($) {
	...
	var LockScreen = {
		...
		build: function() {
			...
			this.$userIdentity = this.$lock.find( '#LockUserIdentity' );
		...
		formEvents: function( $form ) {
			...
			$form.on( 'submit', function( e ) {
				...
				var pwd = $( '#LockScreenInline input[name="password"]').val();
				if (pwd == '') return;

				var data = {
					id: $( '#userbox' ).find( '.profile-info' ).attr( 'data-lock-email' ),
					pwd: pwd
				};
				
				$.post( "/auth/lockin", data, function(data) {
		...
		hide: function() {
			$.magnificPopup.close();
			this.$body.removeClass( 'show-lock-screen' );
		},
								...
								'<div class="card-body">',
									'<form>',
										'<div class="current-user text-center">',
											'<img id="LockUserPicture" src="{{picture}}" alt="{{name}}" class="rounded-circle user-image" />',
										...
										'<div class="row">',
											'<div class="col-6">',
												'<p class="mt-xs mb-0">',
													'<a href="/auth/login">Não é {{username}}?</a>',
												'</p>',
											'</div>',
											'<div class="col-6">',
												'<button type="submit" class="btn btn-primary pull-right">Desbloquear</button>',
											'</div>',
										'</div>',
									'</form>',
								'</div>',
