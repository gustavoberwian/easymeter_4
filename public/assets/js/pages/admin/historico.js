...
//                $('span.badge-log').data('count', 0).addClass("d-none");
	...
	$(document).on('click', '#dt-log .action-readed', function (e) {
        ...
        var id = $btn.data('id');
		// faz a requisição
		$.post("/admin/set_log_state", {id: id}, function(json) {
	...
	$(document).on('click', '#dt-log_paginate .mark-all', function (e) {
		...
		$('#dt-log .actions a').addClass('disabled');
		// faz a requisição
		$.post("/admin/set_log_state", {id: 0}, function(json) {
			if (json.status == 'success') {
				// atualiza tabela
                dtLog.ajax.reload( null, false );
	...
	var dtAccess = $('#dt-access').DataTable({
        ...
        columns: [  { data: "username" },
                    { data: "data", class: "dt-body" },
                    { data: "entidade", className: 'table-ellipsis'}, 
                    { data: "unidade", orderable: false}, 
                    { data: "actions", className: "actions dt-body", orderable: false} ],
        pagingType: "numbers",
        ordering: false,
        autoWidth: false,
        lengthChange: false,
